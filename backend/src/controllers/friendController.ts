...
export const sendFriendRequest = async (req: Request, res: Response): Promise<void> => {
    ...
    if (senderId === receiverId) {
        res.status(400).json({ message: "Bạn không thể tự kết bạn với chính mình." });
        return;
    }
    ...
    try {
        ...
        res.status(201).json({ message: 'Đã gửi lời mời kết bạn.', friendshipId: result.insertId });
    } catch (error) {
...
export const respondToRequest = async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (requests.length === 0) {
            res.status(404).json({ message: "Không tìm thấy lời mời hoặc bạn không có quyền phản hồi."});
