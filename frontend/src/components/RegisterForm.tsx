...
const RegisterForm: React.FC<RegisterFormProps> = ({ switchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data: ApiErrorResponse | RegisterSuccessResponse = await response.json();
  ...
  return (
    ...
    <div className="animate-fade-in">
      <h2 className="text-3xl font-bold text-cyber-purple mb-8 text-center">Tạo tài khoản</h2>
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div className="mb-4">
          <label className="block text-gray-300 mb-2" htmlFor="reg-username">Tên đăng nhập</label>
          <input
            type="text"
            id="reg-username"
            value={username}
        ...
        <div className="mb-4">
          <label className="block text-gray-300 mb-2" htmlFor="reg-email">Email</label>
          <input
            type="email"
            id="reg-email"
            value={email}
        ...
        <div className="mb-4">
          <label className="block text-gray-300 mb-2" htmlFor="reg-nickname">Tên hiển thị (Nickname)</label>
          <input
            type="text"
            id="reg-nickname"
            value={nickname}
        ...
        <div className="mb-6">
          <label className="block text-gray-300 mb-2" htmlFor="reg-password">Mật khẩu</label>
          <input
            type="password"
            id="reg-password"
            value={password}
      ...
      <p className="mt-6 text-center text-gray-400">
        Đã có tài khoản?{' '}
        <button onClick={switchToLogin} className="text-cyber-purple font-semibold hover:underline">
          Đăng nhập
        </button>
      </p>
    </div>
